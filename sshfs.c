...
#define SSHFS_OPT(t, p, v)              \
	{                                   \
		t, offsetof(struct sshfs, p), v \
	}
...
static int buf_get_attrs(struct buffer *buf, struct stat *stbuf, int *flagsp)
{
	...
	if (sshfs.blksize)
	{
		...
		stbuf->st_blocks = ((size + sshfs.blksize - 1) &
							~((unsigned long long)sshfs.blksize - 1)) >>
						   9;
	}
	stbuf->st_uid = uid;
	stbuf->st_gid = gid;
...
<<<<<<< HEAD
	...
	if (!sshfs_file_is_conn(sf))
		return -EIO;
	size_t max_task_size = 1024 * 8; // 32kb
...
>>>>>>> 160ca0c92ba00d7c722ffe82fef9c042c8c626cf
			...
			thpool_add_work(thpool, task_sync_read_func, (void *)task);
...
>>>>>>> 160ca0c92ba00d7c722ffe82fef9c042c8c626cf
			...
			task->size = mytask_size;
			task->offset = current_offset;
			thpool_add_work(thpool, task_async_read_func, (void *)task);
			remaining -= mytask_size;
			current_offset += mytask_size;
		}
		thpool_wait(thpool);
						  ...
						  struct fuse_args *outargs)
{
	...
	case FUSE_OPT_KEY_NONOPT:
		...
		else if (!sshfs.mountpoint)
		{
			 ...
			 */
			if ((('A' <= arg[0] && arg[0] <= 'Z') || ('a' <= arg[0] && arg[0] <= 'z')) && ':' == arg[1] && '\0' == arg[2])
			{
				/* drive: make a copy */
